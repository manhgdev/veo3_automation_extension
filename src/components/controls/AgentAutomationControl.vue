...
const props = defineProps({
  agentAutomationForm: { type: Object, required: true },
...
const uploadedImages = computed({
  get: () => props.agentAutomationForm.images,
  set: (value) => {
    props.agentAutomationForm.images = value;
  },
});
...
const prompts = computed(() => parsePrompts(props.agentAutomationForm.prompt));
...
const maxImagesPerPrompt = computed(() => props.settings.componentsToVideoMaxImagesPerPrompt);
...
function canRunJob() {
  ...
  if (!needsImages.value && !allPromptsHaveImages.value) {
    alert(
      t('agentAutomationControl.validation.noImages', {
...
function jobRunOptions(payloads) {
  return {
    concurrentPrompts: 1,
    promptDelaySecondsMin: props.settings.promptDelaySecondsMin,
    promptDelaySecondsMax: props.settings.promptDelaySecondsMax,
    getGroups: () => props.promptGroups,
    batchIdentity: batchIdentityForJob(props.settings, 'agentAutomation', payloads),
  };
}
...
async function resumePausedGroup(groupId) {
  ...
  await sendJob(payloads, { ...jobRunOptions(payloads), groupId, resumeExisting: true });
}
...
async function runJob() {
  ...
  try {
    await sendJob(payloads, jobRunOptions(payloads));
  } catch (err) {
    toast.add({
      severity: 'error',
...
<template>
  <div class="space-y-4">
    ...
    <div class="flex items-center justify-between gap-3 rounded-lg border border-border/50 px-3 py-2">
      <div>
        <label class="text-xs font-semibold">{{ $t('agentAutomationControl.autoAddCharacterImages.label') }}</label>
        <p class="text-[10px] text-muted-foreground">
          {{ $t('agentAutomationControl.autoAddCharacterImages.description') }}
        </p>
      </div>
    ...
    <div class="space-y-3">
      <PromptTextarea
        v-model="agentAutomationForm.prompt"
        :label="$t('agentAutomationControl.prompts.label')"
        :placeholder="$t('agentAutomationControl.prompts.placeholder')"
        :tip="$t('agentAutomationControl.prompts.tip')"
      />
      ...
      <PromptDurationList
        ...
        :row-warning-text="$t('agentAutomationControl.autoAddCharacterImages.noMatch')"
    ...
    <div class="space-y-2">
      <p class="text-xs text-muted-foreground">{{ $t('agentAutomationControl.advancedHint') }}</p>
      <AutoFileNameToggle :settings="settings" />
    </div>

    <div
      v-if="prompts.length > 0 && !allPromptsHaveImages && !settings.autoAddCharacterImages && !needsImages"
    ...
    <VideoControlFooter
      ...
      :run-label="
        isSending ? $t('agentAutomationControl.buttons.sending') : $t('agentAutomationControl.buttons.run')
      "
      :clear-label="$t('agentAutomationControl.buttons.clear')"
