...
const l = __awaiter;
    ...
    try {
      ...
      if (!o) {
        ...
        if (C([
          ...
          await w(r.modelSelectButton, "AI model select button");
          const jobGroup = Z.find((t) => t.id === e.groupId);
          jobGroup && (jobGroup.flowHasImagen4 = i(r.modelTemplate.replace("{model}", IMAGEN_4_MODEL)).length >
            0);
          const t = r.modelTemplate.replace("{model}", e.model),
            n = await v(t);
          n && n.length > 0 ? await w(t, `Model option: ${e.model}`) : A(
      ...
      if (!c.success) return n && n() ? {
          ...
          for (let i = 0; i < y.length; i++) {
            ...
            for (let u = 1; u <= r; u++) {
              ...
              try {
                ...
                S(`✅ Tile ${i+1}: ${t.autoDownloadResourceQuality} download initiated as ${tileStem}`),
                  c = !0, veoUiDlOk++;
                break
              } catch (a) {
        ...
        for (let e = 0; e < x.length; e++) {
            ...
            if (tileId) try {
              ...
              for (let s = 1; s <= a; s++) {
                try {
                  ...
                  R++, S(`✅ Tile ${e+1}: UI download initiated (1080 fallback) as ${tileStem}`);
                  break
                } catch (c) {
                  s < a && await p(1e3)
                }
          ...
          try {
            ...
            S(`✅ Resource ${t.promptIndex}${variantLabel} downloaded to ${i}/${n}`), R++, await p(500)
          } catch (s) {
  ...
  function ue(e, t, n, o) {
    if (e.pendingDownloads = e.pendingDownloads || {}, e.nextDownloadIndex = e.nextDownloadIndex ?? 0, n) return e
      .downloadItems = e.downloadItems || [], e.downloadItems.push(n), o.push({
        id: e.id,
        items: [n],
        status: "queued",
  ...
  const z = e => new Promise(t => setTimeout(t, e)),
    ...
    veoWaitGenerationCooldown = async (e, t) => {
      ...
      veoCollectFlowTileModelQuotaError = () => {
        try {
          let found = null;
          document.querySelectorAll("[data-tile-id]").forEach((node) => {
            if (found) return;
            const text = (node.textContent || "").replace(/\s+/g, " ").trim();
            if (text && veoIsModelQuotaMessage(text)) found = veoExtractFlowFailMessage(text);
          });
          return found;
        } catch {}
        return null;
      ...
      veoDetectFlowQuotaError = () => {
        try {
          ...
          if (veoIsModelQuotaMessage(surfaceText)) return veoExtractFlowFailMessage(surfaceText);
      ...
      veoTrySwitchModelOnQuota = (jobGroup, message, onUpdate, opts) => {
        ...
        const exhausted = jobGroup.exhaustedModels instanceof Set ? jobGroup.exhaustedModels : new Set(
          jobGroup.exhaustedModels || []);
        const currentModel = extractModelFromQuotaMessage(message) || payload?.model;
        ...
        S(`🔄 Đổi model ${currentModel} → ${nextModel}, tiếp tục group ${jobGroup.id}`);
        return "model-switch";
      },
      veoHandleFlowBlockError = (jobGroup, message, onUpdate, opts) => {
        if (!jobGroup || jobGroup.isCancelling) return null;
        const blockMessage = message != null ? message : veoCollectFlowTileModelQuotaError() ||
          veoDetectFlowQuotaError() || veoDetectFlowFatalError();
  ...
  async function veoProcessDownloadJob(e, t) {
    for (const n of e.items) {
      ...
      if (o.success) {
        ...
        S(`🔄 Model switched — requeue prompt ${n.config?.promptIndex ?? (n.index ?? 0) + 1}`), fe(r, ee)
  ...
  async function veoWaitGroupDownloadsComplete(e, t = 3e5) {
    ...
    for (; !e.isCancelling && Date.now() < n;) {
      const t = ee.some(t => t.id === e.id),
        r = Object.keys(e.pendingDownloads || {}).length > 0,
        o = (e.results || []).some(e => !1 !== e.success && !e.downloadComplete && !e.error);
      if (!t && !r && !o) return !0;
      if (e.isPaused) {
        await K(500);
        continue
