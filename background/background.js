...
async function findFlowTab() {
  ...
  if (!tab || !(await detectFlowPageOnTab(tab))) return null;
  return tab;
}
...
function onDownloadDeterminingFilename(item, suggest) {
  ...
  if (pendingDownloadNames.has(item.url)) {
    suggest({ filename: sanitizeDownloadPath(pendingDownloadNames.get(item.url)) });
  ...
  if (downloadPrefix) {
    suggest({ filename: sanitizeDownloadPath(`${downloadFolder}${downloadPrefix}${ext}`) });
    return;
  }
